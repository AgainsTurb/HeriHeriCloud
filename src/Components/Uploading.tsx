...
listen("upload_progress", (event: any) => {
  ...
  if (!globalLastUpdate[task_id]) {
    globalLastUpdate[task_id] = { time: now, loaded: loaded, speed: "Calculating...", eta: "--" };
  }

  const last = globalLastUpdate[task_id];
  ...
  if (timeDiff >= 0.5 && loaded > last.loaded) {
    ...
    globalLastUpdate[task_id] = { time: now, loaded, speed: speedStr, eta: etaStr };
  }

  const percent = total === 0 ? 0 : Math.round((loaded / total) * 100);
  
  globalProgress[task_id] = { percent, speed: globalLastUpdate[task_id].speed, eta: globalLastUpdate[task_id].eta };
...
export default function Uploading() {
  ...
  const loadTasks = () => {
    ...
    setActiveTasks(active.filter((t: any) => t.type === "Upload"));
  };

  useEffect(() => {
    loadTasks();
    window.addEventListener("TASK_START", loadTasks);
    window.addEventListener("TASK_END", loadTasks);

    const updateUI = () => setProgressMap({ ...globalProgress });
    window.addEventListener("PROGRESS_UPDATE", updateUI);

    return () => {
      window.removeEventListener("TASK_START", loadTasks);
      window.removeEventListener("TASK_END", loadTasks);
      window.removeEventListener("PROGRESS_UPDATE", updateUI);
    };
  }, []);

  async function handleControl(task: any, action: number) {
    if (action === 2 && !confirm(`Cancel upload?`)) return;
    ...
    window.dispatchEvent(action === 0 ? new CustomEvent("TASK_START") : new CustomEvent("TASK_END"));
  }
