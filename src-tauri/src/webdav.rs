...
static URL_CACHE: std::sync::OnceLock<Arc<Mutex<HashMap<u64, CachedMedia>>>> = std::sync::OnceLock::new();
...
async fn fallback_logger(method: axum::http::Method, uri: axum::http::Uri) -> impl IntoResponse {
    println!("\n[WEBDAV-FALLBACK] ⚠️ UNHANDLED PHANTOM PROBE: {} {}", method, uri);
...
async fn resolve_lanzou_media(
    ...
    if node.lanzou_id.starts_with("alien://") {
        ...
        let json_str = crate::lanzou::decrypt_payload(&encoded).map_err(|_| "Failed to decrypt Alien payload".to_string())?;
        let payload: SharePayload = serde_json::from_str(&json_str).map_err(|_| "Failed to parse JSON".to_string())?;
        Ok((node.chunks, payload.l, Some(payload.p).filter(|p| !p.is_empty()), total_size))
    } else {
...
async fn handle_dav_dispatch(
    ...
    for part in parts {
        ...
        println!("[WEBDAV] 🔎 Searching for segment: '{}' (Decoded: '{}') under PID: {}", part, decoded_part, current_id);

        let mut found = None;
        for node in tree.nodes.values() {
            // Check if node belongs to current folder. (Assumes 'pid' is your parent ID property)
            if node.pid == current_id && node.name == decoded_part && !node.is_deleted && !node.is_trashed {
    ...
    match method.as_str() {
            ...
            if depth == "1" && is_dir {
                for child in tree.nodes.values() {
                    if child.pid == current_id && !child.is_deleted && !child.is_trashed {
                        ...
                        let child_path = if p.is_empty() || p == "/" { format!("/{}", safe_name) } else { format!("{}/{}", p, safe_name) };
        ...
        "GET" | "HEAD" => {
            if is_dir { return StatusCode::FORBIDDEN.into_response(); }
            ...
            handle_stream(Path(current_id), AxumState(state.clone()), headers).await.into_response()
        },
        _ => StatusCode::METHOD_NOT_ALLOWED.into_response()
    }
}

fn append_propfind_node(xml: &mut String, is_dir: bool, node: Option<&crate::heriheri::VfsNode>, path: &str) {
    // 1. Format the raw path to ensure it starts with a slash
    let mut raw_path = if path.starts_with('/') { path.to_string() } else { format!("/{}", path) };
...
async fn handle_stream(
    ...
    let media = match cached_media {
        ...
        None => {
            ...
            if chunks_str == "1" || chunks_str.is_empty() {
                let direct_url = match downloader.get_lanzou_direct_link(&share_url, file_pwd.as_deref()).await {
                    Ok(u) => u,
                    Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()).into_response(),
                };
                urls.push(direct_url);
            } else {
                let mut all_files = match downloader.get_lanzou_folder_links(&share_url, file_pwd.as_deref(), 5).await {
                    Ok(f) => f,
                    Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()).into_response(),
                };
                ...
                for file in all_files {
                    let u = file.get("direct_url").and_then(|u| u.as_str()).unwrap_or("").to_string();
                    if u.is_empty() || u == "null" {
                        return (StatusCode::INTERNAL_SERVER_ERROR, "Chunk URL unresolved").into_response();
                    }
                    urls.push(u);
                }
            }
    ...
    let body_stream: BoxStream<'static, Result<axum::body::Bytes, std::io::Error>> = if chunks_str_clone == "1" || chunks_str_clone.is_empty() {
    ...
    if is_partial {
        response_builder = response_builder
            ...
            .header(header::CONTENT_RANGE, format!("bytes {}-{}/{}", start_bytes, end_bytes, total_size))
