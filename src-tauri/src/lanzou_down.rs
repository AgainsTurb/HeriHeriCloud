...
fn acw_sc_v2_simple(arg1: &str) -> String {
    let pos_list = [15,35,29,24,33,16,1,38,10,9,19,31,40,27,22,23,25,13,6,11,39,18,20,8,14,21,32,26,2,30,7,4,17,5,3,28,34,37,12,36];
...
fn get_ajax_headers(referer_url: &str) -> header::HeaderMap {
    ...
    headers.insert("Accept", "application/json, text/javascript, */*".parse().unwrap());
    headers.insert("Accept-Language", "en-US,en;q=0.9,zh-CN;q=0.8,zh;q=0.7".parse().unwrap());
...
impl LanzouDownloader {
    ...
    async fn solve_lanzou_challenge(&self, html: &str, url: &str) -> Result<String, String> {
        ...
        if let Some(caps) = re.captures(html) {
            ...
            self.jar.add_cookie_str(&format!("acw_sc__v2={}; Domain={}; Path=/", cookie_value, domain), &parsed_url);
    ...
    async fn resolve_file_page(&self, share_url: &str, password: Option<&str>) -> Result<(String, String), String> {
        let resp = self.client.get(share_url).send().await.map_err(|e| e.to_string())?;
        let html_initial = resp.text().await.map_err(|e| e.to_string())?;
        let html = self.solve_lanzou_challenge(&html_initial, share_url).await?;

        // Type A
        if html.contains("/ajaxm.php?file=") && html.replace(" ", "").contains("action':'downprocess'") && !html.contains("websignkey") {
            ...
            let sign_re = Regex::new(r"'action'\s*:\s*'downprocess'\s*,\s*'sign'\s*:\s*'([^']+)'").unwrap();

            let file_id = file_id_re.captures(&html_clean).ok_or("Type A: missing file ID")?[1].to_string();
            let sign = sign_re.captures(&html_clean).ok_or("Type A: missing sign")?[1].to_string();

            let ajax_url = Url::parse(share_url).unwrap().join(&format!("/ajaxm.php?file={}", file_id)).unwrap().to_string();
            ...
            let resp = self.client.post(&ajax_url).headers(get_ajax_headers(share_url)).form(&post_data).send().await.map_err(|e| e.to_string())?;
            ...
            let dom = result.get("dom").and_then(|d| d.as_str()).unwrap_or("").to_string();
            let url_path = result.get("url").and_then(|u| u.as_str()).unwrap_or("").to_string();
            return Ok((dom, url_path));
        }
        // Type B
        else if html.contains("/fn?") && !html.contains("wp_sign") {
            let fn_re = Regex::new(r#"src=["'](/fn\?[^"']+)"#).unwrap();
            let fn_match = fn_re.captures(&html).ok_or("Type B: cannot find /fn? URL")?[1].to_string();
            let fn_url = Url::parse(share_url).unwrap().join(&fn_match).unwrap().to_string();

            let mut req_headers = header::HeaderMap::new();
            req_headers.insert("Referer", share_url.parse().unwrap());
            
            let resp = self.client.get(&fn_url).headers(req_headers).send().await.map_err(|e| e.to_string())?;
            ...
            let ajaxdata = ajaxdata_re.captures(&html_fn).ok_or("Type B: missing ajaxdata")?[1].to_string();
            let wp_sign = wp_sign_re.captures(&html_fn).ok_or("Type B: missing wp_sign")?[1].to_string();
            let file_id = file_re.captures(&html_fn).ok_or("Type B: missing file_id")?[1].to_string();

            let killdns_re = Regex::new(r"(var\s+killdns|killdns\s*=)").unwrap();
            let kd = if killdns_re.is_match(&html_fn) { "1" } else { "0" };
            ...
            let resp = self.client.post(&ajax_url).headers(get_ajax_headers(&fn_url)).form(&post_data).send().await.map_err(|e| e.to_string())?;
            let result: Value = resp.json().await.map_err(|e| e.to_string())?;
    ...
    async fn get_direct_link_from_dom_url(&self, dom: &str, url_path: &str, referer: &str) -> Result<String, String> {
        ...
        self.jar.add_cookie_str(&format!("down_ip=1; Domain={}; Path=/", domain), &parsed_dl_url);

        let mut get_headers = header::HeaderMap::new();
        get_headers.insert("Referer", referer.parse().unwrap());

        let resp = self.client.get(&download_page_url).headers(get_headers).send().await.map_err(|e| e.to_string())?;
        let html2 = resp.text().await.map_err(|e| e.to_string())?;
        ...
        let regex_second = Regex::new(r"'file'\s*:\s*'([^']+)'\s*,\s*'el'\s*:\s*[a-zA-Z0-9_]+\s*,\s*'sign'\s*:\s*'([^']+)'").unwrap();
        ...
        let resp = self.client.post(&final_ajax_url)
        ...
        Ok(final_result.get("url").and_then(|u| u.as_str()).unwrap_or("").to_string())
    }

    pub async fn get_lanzou_direct_link(&self, share_url: &str, password: Option<&str>) -> Result<String, String> {
        let (dom, url_path) = self.resolve_file_page(share_url, password).await?;
        self.get_direct_link_from_dom_url(&dom, &url_path, share_url).await
    }

    pub async fn get_lanzou_folder_links(&self, folder_url: &str, password: Option<&str>, concurrency: usize) -> Result<Vec<Value>, String> {
        ...
        let base_file_url = format!("{}://{}", parsed_url.scheme(), parsed_url.host_str().unwrap());
        ...
        loop {
            ...
            let resp = self.client.post(&filemore_url)
        ...
        let mut stream = stream::iter(all_files).map(|file_info| {
            let base_url = base_file_url.clone();
            let dl = downloader.clone();
            
            async move {
                let id = file_info.get("id").and_then(|i| i.as_str()).unwrap_or("").to_string();
                let name = file_info.get("name_all").and_then(|n| n.as_str()).unwrap_or("").to_string();
                let size = file_info.get("size").and_then(|s| s.as_str()).unwrap_or("").to_string();
                let time = file_info.get("time").and_then(|t| t.as_str()).unwrap_or("").to_string();
