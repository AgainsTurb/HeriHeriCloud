...
use crate::heriheri::{VfsTree, VfsNode, get_safe_lanzou_ext, NodeType};
use tauri::Emitter;
...
impl LanzouCloud {
    pub fn new() -> Self {
        ...
        headers.insert(header::USER_AGENT, header::HeaderValue::from_static("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"));
        headers.insert(header::REFERER, header::HeaderValue::from_static("https://up.woozooo.com/mydisk.php"));
    ...
    pub fn set_cookies(&mut self, ylogin: String, phpdisk_info: String) {
        ...
        let mut headers = header::HeaderMap::new();
        headers.insert(header::COOKIE, header::HeaderValue::from_str(&cookie_str).unwrap());
    ...
    async fn post_with_waf(&self, form: &[(&str, &str)]) -> Result<Value, String> {
        ...
        let get_resp = self.client.get(referer_url)
        ...
        let mut post_req = self.client.post(base_url)
            .header("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
        ...
        let resp = post_req.form(&form).send().await.map_err(|e| e.to_string())?;
    ...
    pub async fn init_vfs_root(&mut self) -> Result<String, String> {
        ...
        for f in folders {
            if f["name"].as_str().unwrap_or("") == ".heriheri" {
                ...
                if root_id.is_empty() {
                    if let Some(n) = f["fol_id"].as_u64() { root_id = n.to_string(); }
                }
                break;
            }
        }
        ...
        if root_id.is_empty() {
            ...
            let res = self.create_folder(".heriheri".to_string(), "HeriHeri VFS Root".to_string()).await?;
            
            root_id = res["text"].as_str().unwrap_or("").to_string();
            if root_id.is_empty() {
                if let Some(n) = res["text"].as_u64() { root_id = n.to_string(); }
            }
        }
    ...
    pub async fn list_folders(&self) -> Result<Vec<Value>, String> {
        let folder_id = self.folder_stack.last().unwrap_or(&"-1".to_string()).clone();
        let vei = self.get_vei().await?;
        
        let mut url = format!("{}/doupload.php", BASE_URL);
        if let Some(uid) = &self.ylogin { url.push_str(&format!("?uid={}", uid)); }

        let form = [("task", "47"), ("folder_id", &folder_id), ("vei", &vei)];
        let resp = self.client.post(&url).form(&form).send().await.map_err(|e| e.to_string())?;
        let json: Value = resp.json().await.map_err(|e| e.to_string())?;
    ...
    pub async fn list_files(&self) -> Result<Vec<Value>, String> {
        ...
        loop {
            ...
            let form = [("task", "5"), ("folder_id", &folder_id), ("pg", &pg_str), ("vei", &vei)];
            
            let resp = self.client.post(&url).form(&form).send().await.map_err(|e| e.to_string())?;
            let json: Value = resp.json().await.map_err(|e| e.to_string())?;

            if json["zt"] != 1 || json["info"] == 0 { break; }
            
            if let Some(text_arr) = json["text"].as_array() {
                if text_arr.is_empty() { break; }
    ...
    pub async fn list_folders_by_id(&self, folder_id: &str) -> Result<Vec<Value>, String> {
        ...
        if json["zt"] == 1 { Ok(json["text"].as_array().unwrap_or(&vec![]).clone()) } else { Ok(vec![]) }
    }
    ...
    pub async fn create_folder(&self, folder_name: String, folder_description: String) -> Result<Value, String> {
        let url = format!("{}/doupload.php", BASE_URL);
        let current_id = self.folder_stack.last().unwrap_or(&"-1".to_string()).clone();
        let parent_id = if current_id == "-1" { "0".to_string() } else { current_id };
        ...
        let json: Value = resp.json().await.map_err(|e| e.to_string())?;
        if json["zt"] == 1 { Ok(json) } else { Err("Failed to create folder".to_string()) }
    }
    ...
    pub async fn delete_file(&self, file_id: String) -> Result<bool, String> {
        ...
        let resp = self.client.post(&url).header("X-Requested-With", "XMLHttpRequest").form(&form).send().await.map_err(|e| e.to_string())?;
        let json: Value = resp.json().await.map_err(|e| e.to_string())?;
        Ok(json["zt"] == 1)
    }

    pub async fn delete_folder(&self, folder_id: String) -> Result<bool, String> {
        let clean_id = if folder_id.starts_with("fol") { folder_id.replace("fol", "") } else { folder_id };
        let url = format!("{}/doupload.php", BASE_URL);
    ...
    pub async fn move_item(&self, item_id: String, target_folder_id: String) -> Result<bool, String> {
        let url = format!("{}/doupload.php", BASE_URL);
    ...
    pub async fn restore_item(&self, id: &str, is_folder: bool, formhash: &str) -> Result<bool, String> {
        let url = format!("{}/mydisk.php?item=recycle", BASE_URL);
        let action = if is_folder { "folder_restore" } else { "file_restore" };
        let id_key = if is_folder { "folder_id" } else { "file_id" };
        ...
        let form = [
            ...
            ("task", action),
            (id_key, id),
            ("ref", "https://up.woozooo.com/mydisk.php?item=recycle&action=files"),
            ("formhash", formhash)
        ];
    ...
    pub async fn hard_delete_item(&self, id: &str, is_folder: bool, formhash: &str) -> Result<bool, String> {
        let url = format!("{}/mydisk.php?item=recycle", BASE_URL);
        let action = if is_folder { "folder_delete_complete" } else { "file_delete_complete" };
        let id_key = if is_folder { "folder_id" } else { "file_id" };
    ...
    pub async fn create_folder_in_target(&self, folder_name: String, folder_description: String, parent_id: String) -> Result<Value, String> {
        let url = format!("{}/doupload.php", BASE_URL);
        ...
        let json: Value = resp.json().await.map_err(|e| e.to_string())?;
        if json["zt"] == 1 { Ok(json) } else { Err("Failed to create target folder".to_string()) }
    }

    pub async fn upload_file_direct(&self, bytes: Vec<u8>, safe_name: String, target_folder: String, app: tauri::AppHandle, task_id: String, offset_base: usize, total_file_size: usize, task_flag: Arc<AtomicU8>) -> Result<String, String> {
        let mime = mime_guess::from_path(&safe_name).first_or_octet_stream().to_string();
        ...
        let stream = async_stream::stream! {
            ...
            while offset < total_bytes {
                // MINIMUM FIX: INTERRUPT STREAM IF PAUSED OR CANCELLED (Explicitly typed as Err::<Vec<u8>, _>)
    ...
    pub async fn login(&mut self, username: &str, password: &str) -> Result<(String, String), String> {
        let base_url = "https://accounts.woozooo.com/accounts.php";
        ...
        for cookie in resp.headers().get_all(reqwest::header::SET_COOKIE) {
            if let Ok(c_str) = cookie.to_str() {
                ...
                if c_str.starts_with("phpdisk_info=") {
                    phpdisk_info = c_str.split(';').next().unwrap().replace("phpdisk_info=", "");
                }
            }
        }
    ...
    pub async fn request_register_sms(&self, phone: &str) -> Result<String, String> {
        ...
        if json["zt"] == 1 {
            ...
            Err(json["msgs"].as_str().unwrap_or("Failed to send SMS").to_string())
        }
    }

    pub async fn submit_register(&self, phone: &str, code: &str, password: &str) -> Result<bool, String> {
        // Step 1: Verify the SMS code
        let code_form = [("task", "update_code"), ("phone", phone), ("verycode", code)];
...
fn rebuild_folder_recursive<'a>(tree: &'a mut VfsTree, lanzou: &'a LanzouCloud, node_id: u64, new_parent_pid: u64, new_parent_lanzou_id: String,
    ...
    Box::pin(async move {
        ...
        if node.node_type == NodeType::Directory {
            ...
            let res = lanzou.create_folder_in_target(node.name.clone(), "".to_string(), new_parent_lanzou_id.clone()).await?;
            let cloned_lanzou_id = res["text"].as_str().unwrap_or("").to_string();
            if cloned_lanzou_id.is_empty() { return Err("Failed to create rebuilt folder on cloud".to_string()); }

            // Find all children BEFORE we modify the parent
            let children: Vec<u64> = tree.nodes.values()
            ...
            for child_id in children {
                rebuild_folder_recursive(tree, lanzou, child_id, node_id, cloned_lanzou_id.clone()).await?;
            }
            ...
            // It's a file (or a chunked file folder). Move it natively using Lanzou's API!
            let _ = lanzou.move_item(node.lanzou_id.clone(), new_parent_lanzou_id).await;
...
#[tauri::command]
pub async fn vfs_control_task(task_id: String, action: u8, state: State<'_, AppState>) -> Result<(), String> {
...
#[tauri::command]
pub async fn set_lanzou_cookies(ylogin: String, phpdisk_info: String, state: State<'_, AppState>) -> Result<bool, String> {
...
pub async fn init_vfs_root(app: AppHandle, state: State<'_, AppState>) -> Result<(), String> {
    ...
    let tree_path = app_data_dir.join("heriheri_tree.txt");
...
pub async fn vfs_list_dir(state: State<'_, AppState>) -> Result<Vec<VfsNode>, String> {
    ...
    if let Some(tree) = &*vfs_guard {
        let nodes = tree.list_dir(current_pid).into_iter().filter(|n| !n.is_trashed).collect();
        Ok(nodes)
    } else {
...
#[tauri::command]
pub async fn vfs_create_folder(name: String, desc: String, state: State<'_, AppState>) -> Result<(), String> {
    ...
    if let Some(tree) = vfs_guard.as_mut() {
        ...
        let target_lanzou_folder = if current_pid == 0 {
            ...
            tree.nodes.get(&current_pid).map(|n| n.lanzou_id.clone()).unwrap_or(tree.root_lanzou_id.clone())
        };

        let res = lanzou.create_folder_in_target(name.clone(), desc, target_lanzou_folder).await?;
...
#[tauri::command]
pub async fn vfs_upload_file(file_path: String, task_id: String, target_pid: u64, resume_folder: String, resume_chunk: usize, app: tauri::AppHandle, state: State<'_, AppState>) -> Result<(), String> {
    ...
    let (lanzou_clone, target_lanzou_folder) = {
        ...
        // MINIMUM FIX: Use target_pid instead of the active stack!
    ...
    let _ = app.emit("upload_progress", ProgressPayload { task_id: task_id.clone(), loaded: resume_chunk * chunk_limit, total: total_size });
    ...
    if total_size <= chunk_limit {
        ...
        let res = lanzou_clone.upload_file_direct(bytes, safe_name, target_lanzou_folder, app.clone(), task_id.clone(), 0, total_size, task_flag.clone()).await;
        ...
        final_lanzou_id = if resume_folder.is_empty() {
            let res = lanzou_clone.create_folder_in_target(md5_str.clone(), "".to_string(), target_lanzou_folder).await?;
            res["text"].as_str().unwrap_or("").to_string()
        } else {
            resume_folder.clone()
        };
        
        if final_lanzou_id.is_empty() { return Err("Failed to create chunk directory on cloud".to_string()); }
        ...
        for i in resume_chunk..num_chunks {
            ...
            let upload_res = lanzou_clone.upload_file_direct(
                chunk_bytes, chunk_name, final_lanzou_id.clone(), app.clone(), task_id.clone(), current_loaded, total_size, task_flag.clone()
            ).await;
    ...
    let mut vfs_guard = state.vfs.lock().await;
    if let Some(tree) = vfs_guard.as_mut() {
        // MINIMUM FIX: Save to target_pid
...
pub async fn vfs_delete_item(id: u64, state: tauri::State<'_, AppState>) -> Result<bool, String> {
    ...
    if let Some(tree) = vfs_guard.as_mut() {
        let (lanzou_id, is_physical_folder) = {
            ...
            let is_chunked = node.node_type == crate::heriheri::NodeType::File && node.chunks != "1" && !node.chunks.is_empty();
...
pub async fn vfs_get_share_info(id: u64, state: State<'_, AppState>) -> Result<Value, String> {
    ...
    if let Some(tree) = &*vfs_guard {
        ...
        let is_physical_folder = node.node_type == NodeType::Directory || (node.node_type == NodeType::File && node.chunks != "1" && !node.chunks.is_empty());
...
#[tauri::command]
pub async fn login(username: String, password: String, state: State<'_, AppState>) -> Result<(String, String), String> {
    let mut lanzou = state.lanzou.lock().await;
    lanzou.login(&username, &password).await
}

#[tauri::command]
pub async fn request_register_sms(phone: String, state: State<'_, AppState>) -> Result<String, String> {
    let lanzou = state.lanzou.lock().await;
    lanzou.request_register_sms(&phone).await
}

#[tauri::command]
pub async fn submit_register(phone: String, code: String, password: String, state: State<'_, AppState>) -> Result<bool, String> {
    let lanzou = state.lanzou.lock().await;
...
async fn internal_create_folder(name: &str, target_pid: u64, state: &AppState) -> Result<u64, String> {
    let lanzou = state.lanzou.lock().await;
    let mut vfs_guard = state.vfs.lock().await;
    ...
    if let Some(tree) = vfs_guard.as_mut() {
        ...
        let res = lanzou.create_folder_in_target(name.to_string(), "".to_string(), target_lanzou_folder).await?;
        let new_lanzou_id = res["text"].as_str().unwrap_or("").to_string();
...
#[tauri::command]
pub async fn vfs_expand_drop(paths: Vec<String>, current_pid: u64, state: tauri::State<'_, AppState>) -> Result<Vec<DropFile>, String> {
    let mut result = Vec::new();
    
    for path_str in paths {
        let path = std::path::Path::new(&path_str);
        if path.is_file() {
            result.push(DropFile { path: path_str, target_pid: current_pid, group_id: None, group_name: None });
        } else if path.is_dir() {
            ...
            let group_id = Some(format!("g_{}_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_millis(), folder_pid));
                ...
                if let Ok(entries) = std::fs::read_dir(&current_dir) {
                    for entry in entries.flatten() {
                        ...
                        if p.is_file() || p.is_symlink() {
                            ...
                            let child_name = p.file_name().unwrap_or_default().to_string_lossy().to_string();
                            let child_pid = match internal_create_folder(&child_name, pid, &state).await {
                                Ok(id) => id,
                                Err(_) => pid,
                            };
    ...
    if let Some(tree) = &*vfs_guard {
        ...
        while current != 0 {
            if let Some(node) = tree.nodes.get(&current) {
                path.push(Breadcrumb { id: current, name: node.name.clone() });
                current = node.pid; 
            } else {
                break;
            }
        ...
        path.push(Breadcrumb { id: 0, name: "All Files".to_string() });
        path.reverse();
...
#[tauri::command]
pub async fn vfs_batch_delete(ids: Vec<u64>, state: tauri::State<'_, AppState>) -> Result<bool, String> {
    let lanzou = state.lanzou.lock().await;
    let mut vfs_guard = state.vfs.lock().await;
...
#[tauri::command]
pub async fn vfs_move_items(item_ids: Vec<u64>, target_pid: u64, state: tauri::State<'_, AppState>) -> Result<bool, String> {
    let lanzou = state.lanzou.lock().await;
    let mut vfs_guard = state.vfs.lock().await;
    ...
    if let Some(tree) = vfs_guard.as_mut() {
        ...
        for id in item_ids {
            ...
            if let Some(NodeType::Directory) = node_type {
                ...
                rebuild_folder_recursive(tree, &lanzou, id, target_pid, target_lanzou_id.clone()).await?;
            } else if let Some(node) = tree.nodes.get(&id).cloned() {
                // If it's a file, execute fast move on the cloud
...
#[tauri::command]
pub async fn vfs_list_bin(state: tauri::State<'_, AppState>) -> Result<Vec<crate::heriheri::VfsNode>, String> {
    let vfs_guard = state.vfs.lock().await;
    if let Some(tree) = &*vfs_guard {
        let bin_nodes: Vec<crate::heriheri::VfsNode> = tree.nodes.values()
...
#[tauri::command]
pub async fn vfs_restore_items(ids: Vec<u64>, state: tauri::State<'_, AppState>) -> Result<bool, String> {
    let lanzou = state.lanzou.lock().await;
    ...
    if let Some(tree) = vfs_guard.as_mut() {
        ...
        for id in targets {
            if let Some(node) = tree.nodes.get(&id).cloned() {
                ...
                if lanzou.restore_item(&node.lanzou_id, is_physical_folder, &formhash).await.unwrap_or(false) {
                    if !is_physical_folder {
...
#[tauri::command]
pub async fn vfs_hard_delete_items(ids: Vec<u64>, state: tauri::State<'_, AppState>) -> Result<bool, String> {
    let lanzou = state.lanzou.lock().await;
    let formhash = lanzou.get_formhash().await?;
    let mut vfs_guard = state.vfs.lock().await;
...
#[tauri::command]
pub async fn vfs_rename_item(id: u64, new_name: String, state: tauri::State<'_, AppState>) -> Result<bool, String> {
    let mut vfs_guard = state.vfs.lock().await;
...
fn flatten_tree(tree: &VfsTree, pid: u64, current_path: String, out: &mut Vec<FlatFile>) {
    let children: Vec<_> = tree.nodes.values().filter(|n| n.pid == pid).cloned().collect();
    for node in children {
        let node_path = if current_path.is_empty() { node.name.clone() } else { format!("{}/{}", current_path, node.name) };
        let is_chunked = node.node_type == NodeType::File && node.chunks != "1" && !node.chunks.is_empty();
        
        if node.node_type == NodeType::File || is_chunked {
            out.push(FlatFile { id: node.id, name: node.name, size: node.size, rel_path: node_path });
...
#[tauri::command]
pub async fn vfs_get_folder_tree(id: u64, state: tauri::State<'_, AppState>) -> Result<Vec<FlatFile>, String> {
    let vfs_guard = state.vfs.lock().await;
    if let Some(tree) = &*vfs_guard {
        let mut out = Vec::new();
...
#[tauri::command]
pub async fn vfs_download_file(task_id: String, vfs_id: u64, local_path: String, resume_offset: usize, total_size: usize, app: tauri::AppHandle, state: State<'_, AppState>) -> Result<(), String> {
    let task_flag = Arc::new(AtomicU8::new(0));
    state.task_ctrl.lock().await.insert(task_id.clone(), task_flag.clone());

    let (node, share_url, file_pwd) = {
        let vfs_guard = state.vfs.lock().await;
        let tree = vfs_guard.as_ref().ok_or("VFS Offline")?;
        let node = tree.nodes.get(&vfs_id).cloned().ok_or("Node not found")?;
        
        let is_folder = node.node_type == crate::heriheri::NodeType::Directory || (node.chunks != "1" && !node.chunks.is_empty());
    ...
    if node.chunks == "1" || node.chunks.is_empty() {
    ...
    // --- CASE B: CHUNKED FILE REASSEMBLY ---
        ...
        let mut all_files = downloader.get_lanzou_folder_links(&share_url, file_pwd.as_deref(), 5).await?;
        ...
        for i in start_chunk_idx..all_files.len() {
            let direct_url = all_files[i].get("direct_url").and_then(|u| u.as_str()).unwrap_or("");
            
            if direct_url.is_empty() || direct_url == "null" {
                return Err(format!("Failed to resolve direct URL for chunk: {}", all_files[i].get("error").and_then(|e| e.as_str()).unwrap_or("Unknown")));
            }
            ...
            if !resp.status().is_success() {
                return Err(format!("CDN Error on Chunk {}: HTTP {}", i + 1, resp.status()));
            }
...
async fn get_sync_folder_id(lanzou: &LanzouCloud) -> Result<String, String> {
    ...
    for f in folders {
        if f["name"].as_str().unwrap_or("") == ".vfs" {
            ...
            if let Some(n) = f["fol_id"].as_u64() { return Ok(n.to_string()); }
        }
    }

    let res = lanzou.create_folder_in_target(".vfs".to_string(), "HeriHeri Sync Data".to_string(), root_id.to_string()).await?;
    let new_id = res["text"].as_str().unwrap_or("").to_string();
    if !new_id.is_empty() { return Ok(new_id); }
    if let Some(n) = res["text"].as_u64() { return Ok(n.to_string()); }
...
async fn execute_sync_pull(state: &AppState) -> Result<bool, String> {
    ...
    let (lanzou, downloader, local_timestamp, local_path) = {
        ...
        let tree = vfs_guard.as_ref().ok_or("VFS Offline")?;
        (
            lanzou_guard.clone(), down_guard.clone(), 
            tree.last_modified, tree.file_path.clone()
        )
    };
    ...
    let re = Regex::new(r"heriheri_tree_(\d+)\.txt").unwrap();
    ...
    println!("[SYNC] Cloud is newer ({} > {}). Pulling state...", highest_cloud_ts, local_timestamp);

    let share_info = lanzou.get_share_info(target_file_id.clone(), false).await?;
    let share_url = if let Some(u) = share_info["new_url"].as_str() { u.to_string() } else { format!("{}/{}", share_info["is_newd"].as_str().unwrap_or(""), share_info["f_id"].as_str().unwrap_or("")) };
    ...
    let resp = req_client.get(&direct_url).send().await.map_err(|e| e.to_string())?;
    
    if !resp.status().is_success() { return Err("Failed to download cloud TSV".to_string()); }
...
#[tauri::command]
pub async fn vfs_sync_push(app: AppHandle, state: tauri::State<'_, AppState>) -> Result<bool, String> {
    let _guard = state.sync_lock.lock().await;
    ...
    let file_name = format!("heriheri_tree_{}.txt", new_timestamp);
    
    println!("[SYNC] Pushing new state to cloud: {}", file_name);

    let dummy_flag = Arc::new(AtomicU8::new(0));
    let upload_res = lanzou.upload_file_direct(
        tsv_bytes, file_name.clone(), sync_folder_id.clone(), 
        app, "SYNC_TASK".to_string(), 0, total_size, dummy_flag
    ).await;
    ...
    for f in files_after {
        if let Some(name) = f["name"].as_str() {
            if name.starts_with("heriheri_tree_") {
