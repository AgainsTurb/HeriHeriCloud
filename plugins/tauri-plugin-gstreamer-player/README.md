...
Manual preparation remains available through `npm run setup:gstreamer`, or the platform-specific `setup:gstreamer:*` scripts. Desktop `tauri build` commands automatically stage a playback-focused runtime: Windows installers carry the recursively selected DLL/plugin closure, macOS apps carry rewritten universal dylibs and plugins under their signed bundle, and AppImage feeds only the selected plugins/scanner into Tauri's media-framework bundler so linuxdeploy can collect their shared dependencies. Debian packages retain Tauri's generated library dependencies and add the dynamically loaded GStreamer codec/sink packages as hard dependencies. Development commands continue using the configured SDK rather than copying release assets. Final desktop installer artifacts are rejected above 300 MiB before release upload.
