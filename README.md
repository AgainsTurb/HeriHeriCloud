...
1.  点击项目页面右上角的 “Fork” 按钮，将仓库 Fork 到你的 GitHub 账户下。
...
1.  访问你 Fork 后的 GitHub 仓库页面，通常会看到一个提示你发起 Pull Request 的横幅，点击 “Compare & pull request”。
    
2.  确保 base 分支选择原始仓库的 `main`（或 `master`），compare 分支选择你的功能分支。
    
3.  填写清晰的标题和详细的改动说明，如有相关 Issue 也请提及。
    
4.  提交 Pull Request。维护者会进行审查，可能需要你进一步修改。
...
We warmly welcome contributions from the community, whether it's feature development, bug fixing, or UI improvements.
...
Discussions in GitHub Issues and Pull Requests are welcome in any language (Chinese or English).
...
Always create a new branch for your work. Choose a descriptive branch name:  
```bash
...
-   Keep changes focused and well‑documented (comments in English).
...
2.  Commit with a clear, concise message (English is preferred for commit messages): 
```bash
git commit -m "Fix errors"
```
...
1.  Visit your fork on GitHub. You will see a banner prompting you to open a Pull Request. Click "Compare & pull request".
    
2.  Ensure the base branch is the original repository’s `main` (or `master`) and the compare branch is your feature branch.
    
3.  Write a descriptive title and a clear explanation of your changes. Mention any related issues.
    
4.  Submit the Pull Request. The maintainers will review it and may request further adjustments.
